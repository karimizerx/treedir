...
int main(int argc, char const *argv[])
{
    ...
    noeud ncourant = *n0;
    noeud *courant = &ncourant;
    printf("Noeud courant : ");
    pwd(courant);
    cd(&courant, "Cours/ProjetC");
    printf("Noeud courant après cd 'Cours/ProjetC' : ");
    pwd(courant);
    cd(&courant, "/Cours");
    ...
    *n0 = init_noeud(n0, true, nom0);     // On crée le noeud racine.
    (*n0)->pere = *n0;
    (*n0)->racine = *n0;
    read(n0, "test6.txt");     // On lit le fichier contenant la liste des commandes & on exécute celles-ci.
